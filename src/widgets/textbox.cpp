...
float LGF::Widgets::TextBox::getCornerRadius() {
    return this->radius;
}

LGF::Widgets::TextBox& LGF::Widgets::TextBox::setCornerRadius(float radius) {
    this->radius = radius;
    return *this;
}

LGF::Widgets::TextBox& LGF::Widgets::TextBox::linkContentLabel(LGF::Widgets::Label* label) {
    this->text_content = label;
    this->addChild(label);
    return *this;
}

LGF::Widgets::TextBox::TextBox(LGF::LGFWindow* window) :
    ...
    this->window->onChar += [&] {
        if (focused) {
            if (text_content == nullptr) {
                std::cout << "The text content label hasn't been linked to the textbox!\n";
                return;
            }
            text_content->setText(this->window->getTextString());
        }
    };
...
LGF::Widgets::TextBox& LGF::Widgets::TextBox::setMinimumSize(const glm::vec2& size) {
    this->minSize = size;
    return *this;
}
LGF::Widgets::TextBox& LGF::Widgets::TextBox::setMaximumSize(const glm::vec2& size) {
    this->maxSize = size;
    return *this;
}

void LGF::Widgets::TextBox::updatePanel() {
    if (!enabled) return;
...
LGF::Widgets::TextBox& LGF::Widgets::TextBox::setRect(const glm::vec2& pos, const glm::vec2& size) {
    this->position = pos;
    this->size = size;
...
LGF::Widgets::TextBox& LGF::Widgets::TextBox::setColour(const int& r, const int& g, const int& b, const int& a) {
